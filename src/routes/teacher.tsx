...
export const Route = createFileRoute("/teacher")({
  ...
  head: () => ({
    meta: [
      { title: `Teacher Portal — ${SCHOOL_NAME} (EduCard Pro)` },
      { name: "description", content: "Grade entry, class attendance, and analytics for teachers." },
    ],
  }),
});
...
const classAvgBySubject = subjects.map((subj) => ({
  subject: subj,
  avg: Math.round(
    mySectionLearners.reduce((a, l) => a + (subjectGrades[l.lrn]?.[subj] ?? 85), 0) /
